...
const Vec2 PARTICLE_POS = V2(-80, 15);

f32 CLOUD_SIZE = 4;

void setCloudSys(Renderer::ParticleSystem& sys) {
    sys = Renderer::create_particle_system(20, PARTICLE_POS);
    sys.one_alpha = true;
    sys.one_size = true;
    sys.relative = true;
    sys.alive_time = {200, 200};
    sys.rotation = {PI, PI};
    sys.velocity_dir = {};
    sys.velocity = {5, 10};
    sys.damping = {1, 1};
...
void createCloudSystems() {
    ...
    cloud_particles_1.velocity = {3, 6};
    ...
    cloud_particles_2.velocity = {5, 10};
    ...
    cloud_particles_3.velocity = {3, 6};
    ...
    cloud_particles_4.velocity = {2, 4};
    ...
    cloud_particles_5.velocity = {3, 6};
}
...
void updateClouds(f32 delta) {
    cloud_particles_1.position = paralax(PARTICLE_POS, 23);
    cloud_particles_2.position = paralax(PARTICLE_POS, 27);
    cloud_particles_3.position = paralax(PARTICLE_POS, 21);
    cloud_particles_4.position = paralax(PARTICLE_POS, 10);
    cloud_particles_5.position = paralax(PARTICLE_POS, 19);
